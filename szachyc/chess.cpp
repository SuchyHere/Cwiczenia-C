...
bool Board::doMove() {
    ...
    while (!stop)
    {
        ...
        if (move.length() != 4 || !isdigit(move[0]) || !isdigit(move[1]) || !isdigit(move[2]) || !isdigit(move[3])) {
            cout << "Invalid input. Please enter a four character string using numeric coordinates." << endl;
            continue;
        }
...
bool Board::makeMove(int x1, int y1, int x2, int y2) {
    ...
    switch (src->getPiece()) {
        case KING: moveSuccessful = moveKing(src, dest); break;
        case QUEEN: moveSuccessful = moveQueen(src, dest); break;
        case BISHOP: moveSuccessful = moveBishop(src, dest); break;
        case KNIGHT: moveSuccessful = moveKnight(src, dest); break;
        case ROOK: moveSuccessful = moveRook(src, dest); break;
        case PAWN: moveSuccessful = movePawn(src, dest); break;
        case EMPTY: cout << "You do not have a piece there" << endl; return false;
        default: cerr << "Something went wrong in the switch statement in makeMove()" << endl; return false;
    }
